...
from ..utils.ml.eval import evaluate_local_score_model, evaluate_unc_local_score_model, evaluate_repulsive_ensemble_local_score_model
from ..utils.ml.models.score import DenseLocalScoreModel, HeteroSkedasticDenseLocalScoreModel, BayesianDenseLocalScoreModel, RepulsiveEnsembleDenseLocalScoreModel
from ..utils.ml.trainer import LocalScoreTrainer, BayesianLocalScoreTrainer, RepulsiveEnsembeLocalScoreTrainer
...
class RepulsiveEnsembleScoreEstimator(ScoreEstimator):
    def __init__(
        self, 
        features=None, 
        n_hidden=(100,), 
    ...
    def train(
        ...
        # Train model
        logger.info("Training model")
        trainer = RepulsiveEnsembeLocalScoreTrainer(self.model, n_workers=n_workers)
        result = trainer.train(
            data=data,
            data_val=data_val,
            loss_functions=loss_functions,
...
class BayesianScoreEstimator(ScoreEstimator):
    def train(
        ...
        if method not in ["bayesian_sally"]:
            logger.warning("Method %s not allowed for score estimators. Using 'sally' instead.", method)
        ...
        # Train model
        logger.info("Training model")
        trainer = BayesianLocalScoreTrainer(self.model, n_workers=n_workers)
        result = trainer.train(
            data=data,
            data_val=data_val,
            loss_functions=loss_functions,
    ...
    def _create_model(self):
        self.model = BayesianDenseLocalScoreModel(
            n_observables=self.n_observables,
            n_parameters=self.n_parameters,
            n_hidden=self.n_hidden,
            activation=self.activation,
