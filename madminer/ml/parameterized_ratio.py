...
from ..utils.ml.models.ratio import DenseSingleParameterizedRatioModel, MorphDenseSingleParameterizedRatioModel
from ..utils.ml.trainer import SingleParameterizedRatioTrainer, MorphSingleParameterizedRatioTrainer
...
class MorphParameterizedRatioEstimator(ConditionalEstimator):
    """
    """
    ...
    def train(
        ...
        # Train model
        logger.info("Training model")
        trainer = MorphSingleParameterizedRatioTrainer(self.model, n_workers=n_workers)
        result = trainer.train(
            data=data,
            data_val=data_val,
            loss_functions=loss_functions,
    ...
    def _create_model(self):
        self.model = MorphDenseSingleParameterizedRatioModel(
            n_observables=self.n_observables,
            n_hidden=self.n_hidden,
            activation=self.activation,
            dropout_prob=self.dropout_prob,
    ...
    @staticmethod
    def _package_training_data(method, x, y, r_xz, t_xz):
        data = OrderedDict()
        data["x"] = x
        data["y"] = y
        if method in ["rolr", "alice", "alices", "rascal"]:
